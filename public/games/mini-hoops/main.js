const W=960,H=540; const config={ type:Phaser.AUTO,parent:'game',width:W,height:H,backgroundColor:'#1b2246', physics:{default:'arcade', arcade:{gravity:{y:1000}}}, scene:{preload,create,update} };
let ball, rim, score=0, timeLeft=60, dragging=false, startPos=null, paused=false, timerEvt, lastScoreAt=0;
function preload(){}
function create(){
  window.__scene=this;
  ...
  ball=this.physics.add.circle(W/2,H-60,18,0xffe066); ball.setBounce(0.7); ball.setCollideWorldBounds(true);
  this.physics.add.collider(ball, rim);
  this.input.on('pointerdown',(p)=>{ dragging=true; startPos={x:p.x,y:p.y}; });
  this.input.on('pointerup',(p)=>{ if(!dragging) return; dragging=false; const dx=p.x-startPos.x, dy=p.y-startPos.y; ball.setVelocity(dx*3, dy*3); });
  const zone=this.add.zone(W/2,260,100,40); this.physics.world.enable(zone); zone.body.setAllowGravity(false); zone.body.moves=false; this.physics.add.overlap(ball, zone, scored, null, this);
  timerEvt=this.time.addEvent({ delay:1000, loop:true, callback:()=>{ if(!paused){ timeLeft--; updateHUD(); if(timeLeft<=0) gameOver(); } } });
  updateHUD();
}
function scored(){ const now=performance.now(); if(now-lastScoreAt<600) return; lastScoreAt=now; score+=2; updateHUD(); }
...
function gameOver(){ alert('Time up! Score: '+score); location.reload(); }
function update(){}
